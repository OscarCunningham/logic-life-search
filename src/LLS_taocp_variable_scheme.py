...
def definition_clauses(grid, x, y, t, letter, at_least):
    ...
    if letter == None:
        ...
        clauses = []  # We'll build up a list of the clauses we need one at a time
        ...
        if at_least <= 0:
            ...
            clauses.append([negate(literal_name(grid, x, y, t, letter, at_least))])

        # Otherwise define the appropriate clauses
        else:
            if at_least <= maximum_number_of_live_cells_1:
                clauses.append([negate(literal_name(grid, child_1_x, child_1_y, t, child_1_letter, at_least)),
            ...
            if at_least <= maximum_number_of_live_cells_2:
                clauses.append([negate(literal_name(grid, child_2_x, child_2_y, t, child_2_letter, at_least)),
                                literal_name(grid, x, y, t, letter, at_least)])
                child_2_needing_definition.append(at_least)

            if at_least > maximum_number_of_live_cells_2:
                i = at_least - maximum_number_of_live_cells_2
                clauses.append([literal_name(grid, child_1_x, child_1_y, t, child_1_letter, i),
            ...
            if at_least > maximum_number_of_live_cells_1:
                ...
                clauses.append([literal_name(grid, child_2_x, child_2_y, t, child_2_letter, j),
                                negate(literal_name(grid, x, y, t, letter, at_least))])
                child_2_needing_definition.append(j)
        ...
        for child_1_at_least in child_1_needing_definition:
            clauses += definition_clauses(grid, child_1_x, child_1_y, t, child_1_letter, child_1_at_least)
        for child_2_at_least in child_2_needing_definition:
            clauses += definition_clauses(grid, child_2_x, child_2_y, t, child_2_letter, child_2_at_least)
...
def transition_rule(grid, x, y, t):
    ...
    clauses.append(implies(
        literal_name(grid, x, y, t - 1, "a", at_least = 4),
        negate(cell)))
    # If there aren't at least 2 neighbours in the previous generation then the cell dies
    clauses.append(implies(
        negate(literal_name(grid, x, y, t - 1, "a", at_least = 2)),
        negate(cell)))
